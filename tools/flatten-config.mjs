...
function stripExports(content, absPath) {
  ...
  content = content.replace(/export\s+default\s+function\s*\(/g, m => {
  ...
  // export default <expr>;
  content = content.replace(/export\s+default\s+([^;]+);?/g, (_m, expr) => {
    const name = deriveNameFromFilename(absPath) || 'defaultExport'
    declared.push(name)
  ...
  // export { a, b as c }
  content = content.replace(/export\s*\{[\s\S]*?\};?/g, m => {
...
function splitHeaderBody(eleventyContent) {
  const match = eleventyContent.match(
    /^[\s\S]*?export\s+default\s+function\s*\(/m
  )
  if (!match)
    return { header: eleventyContent, body: '', idx: eleventyContent.length }
...
async function main() {
  ...
  const importRx =
    /^\s*import\s+[\s\S]+?from\s+['"](?:\.{1,2}\/)?config\/[^'"]+['"];?\s*$/gm
  ...
  // Compose final file
  const banner = `// eleventy.config.mjs (FLATTENED)
