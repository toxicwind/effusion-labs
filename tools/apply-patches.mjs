...
if (args.has('--help')) {
  console.log(`Usage: node tools/apply-patches.mjs [--check] [--force] [--verbose]\n\n` +
...
async function main() {
  ...
  for (const patchName of patchFiles) {
    ...
    if (checkResult.code === 0) {
      ...
      await runGit(['apply', '--ignore-space-change', '--whitespace=nowarn', relPath], {stream: verbose});
      if (!verbose) {
    ...
    const reverseCheck = await runGit(['apply', '--reverse', '--check', relPath], {allowFailure: true});
