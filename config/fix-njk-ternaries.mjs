...
function convertExpr(expr) {
    ...
    let depth = 0, quote = null, q = -1, c = -1;
    for (let i = 0; i < expr.length; i++) {
        ...
        else if (depth === 0 && ch === ':' && q !== -1) { c = i; break; }
    }
...
function rewriteContent(content) {
    ...
    const out = content.replace(re, (m, inner) => {
        let prev = inner, next = null, iterations = 0;
...
export async function fixRepoTernaries({ roots = ['src'], exts = ['.njk'], dryRun = false, logFile = '.njk-fix-report.json' } = {}) {
...
if (import.meta.url === `file://${process.argv[1]}`) {
    ...
    fixRepoTernaries({ roots: ['src'], exts: ['.njk', '.md', '.html'], dryRun })
