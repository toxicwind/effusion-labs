...
const ensureDir = async d => {
  if (!(await exists(d))) await fsp.mkdir(d, { recursive: true })
}
...
function noteConflict(what, why) {
  CONFLICTS.push({ what, why })
}
function noteSkip(what, why) {
...
async function computeMoves() {
  ...
  if (await exists(LAYOUT_SRC))
    planMove(LAYOUT_SRC, LAYOUT_DST, 'normalize base layout')
  ...
  for (const [fromRel, toRel] of endpoints) {
    ...
    if (await exists(fromAbs))
      planMove(fromAbs, path.join(SRC, toRel), 'endpoint → pages/')
  }
  ...
  if (await exists(RESUME_SRC))
    planMove(
      RESUME_SRC,
      path.join(SRC, 'pages', 'resume', 'index.njk'),
      'resume endpoint'
    )
  if (await exists(RESUME_CARD_SRC))
    planMove(
      RESUME_CARD_SRC,
      path.join(SRC, 'pages', 'resume', 'card.njk'),
  ...
  if (await exists(EMBED_SRC))
    planMove(
      EMBED_SRC,
      path.join(SRC, '_includes', 'components', 'embed.njk'),
      'component, not layout'
    )
  const ARCHIVE_NAV_SRC = path.join(SRC, '_includes', 'archive-nav.njk')
  if (await exists(ARCHIVE_NAV_SRC))
    planMove(
      ARCHIVE_NAV_SRC,
      path.join(SRC, '_includes', 'components', 'archive-nav.njk'),
      'component placement'
    )
  ...
  if (await exists(SRC_SCRIPTS)) {
    ...
    for (const f of files) {
      ...
      if ((await fsp.stat(fromAbs)).isFile()) {
        planMove(
          fromAbs,
          path.join(SRC, 'assets', 'js', f),
          'js authoring → assets/js'
        )
      }
    }
  }
  ...
  if (await exists(OG_IMG))
    planMove(
      OG_IMG,
      path.join(SRC, 'assets', 'images', 'og', 'og-ai-safety.jpg'),
      'group OG images'
    )
  ...
  if (await exists(LOGO))
    planMove(
      LOGO,
      path.join(SRC, 'assets', 'images', 'logo.png'),
      'collapse static/ into images/'
    )
}
...
async function applyMoves() {
  ...
  for (const m of MOVES) {
    ...
    const toAbs = path.join(ROOT, m.to)
    if (!(await exists(fromAbs))) {
      noteSkip(m.from, 'missing source')
      continue
    }
...
async function scanAndRewrite() {
  for await (const p of walk(SRC)) {
    ...
    const njkRules = [
      {
        re: /(\{%\s*(?:include|from)\s+["'])archive-nav\.njk(["'])/g,
        to: `$1components/archive-nav.njk$2`,
      },
      {
        re: /(\{%\s*(?:include|extends|from)\s+["'])layouts\/embed\.njk(["'])/g,
        to: `$1components/embed.njk$2`,
      },
      {
        re: /(\{%\s*extends\s+["'])layout\.njk(["'])/g,
        to: `$1layouts/base.njk$2`,
      },
    ]
    ...
    if (p.endsWith('.11ty.js') || p.endsWith('.js')) {
      ...
      if (j.changed) {
        ...
        planRewrite(
          p,
          'js:data.layout',
          `layout: 'base.njk' → 'layouts/base.njk'`
        )
      }
    }

    // 4) src-only script import path (do NOT touch '/scripts/' web root)
    const jsRules = [{ re: /(["'`])src\/scripts\//g, to: `$1src/assets/js/` }]
    ...
    const assetRules = [
      {
        re: /\/assets\/og-ai-safety\.jpg\b/g,
        to: `/assets/images/og/og-ai-safety.jpg`,
      },
      {
        re: /(?<!:)assets\/static\/logo\.png\b/g,
...
function printReport(foundCount) {
  ...
  // Rewrites by kind
  const byKind = rewrites.reduce(
    (acc, r) => (((acc[r.kind] ||= 0), acc[r.kind]++), acc),
    {}
  )
  console.log('\n› Rewrites (by kind)\n')
  if (rewrites.length) {
...
;(async function main() {
  ...
  if (!DRY && !gitClean()) {
    console.error(
      'Refusing to run with --do-it: working tree is dirty. Commit/stash first.'
    )
    process.exit(1)
  }
  ...
  if (DRY) {
    ...
    console.log(
      'Reminder: ensure Eleventy copies "src/assets" → "assets" (passthrough) after this refactor.'
    )
  } else {
    console.log(
      'APPLIED. Consider running: npx @11ty/eleventy && rg -n "layout\\.njk|layouts\\/embed\\.njk|archive-nav\\.njk|src\\/scripts\\/" src/'
    )
  }
})().catch(err => {
  console.error(err)
