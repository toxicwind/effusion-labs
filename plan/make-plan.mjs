...
function discoverByFS() {
  ...
  function pickIncludes() {
    const cands = [
      path.join(input, '_includes'),
      '_includes',
      path.join(input, 'includes'),
    ]
  ...
  return {
    ...
    includes,
    data,
    formats: TEMPLATE_FORMATS.slice(),
    output: '_site',
  }
}
...
async function writeTreeBefore(files) {
  ...
  await fsp.writeFile(
    path.join(inspectDir, 'tree.before.txt'),
    files.join('\n')
  )
}
...
async function writeMoveMapDocs() {
  ...
  for (const intent of intents) {
    ...
    if (haveCmd('fd', ['--version'])) {
      const r = spawnSync(
        'fd',
        ['--type', 'f', '--strip-cwd-prefix', '.', intent.src],
        { cwd: repoRoot, encoding: 'utf8' }
      )
      if (r.status === 0) {
        for (const rel of r.stdout.split('\n').filter(Boolean)) {
          rows.push(
            `${path.join(intent.src, rel)}\t${path.join(intent.dest, rel)}`
          )
        }
        continue
      }
    ...
    const walk = (dir, relBase = '') => {
      ...
      for (const e of entries) {
        ...
        if (stat.isDirectory()) walk(full, path.join(relBase, e))
          ...
          rows.push(`${full}\t${path.join(intent.dest, relBase, e)}`)
      }
    }
    walk(intent.src)
  }
...
async function planSrcNormalization() {
  ...
  for (const ent of entries) {
    ...
    if (name.startsWith('.')) {
      conflicts.skipped.push(path.join('src/assets/css', name))
      continue
    }
    if (name === 'app.css') {
      conflicts.skipped.push(path.join('src/assets/css', name))
...
async function writeRewritesSed(docPairs, cssPairs) {
  ...
  const lines = pairs.map(
    ([from, to]) => `s|${escapeSedLiteral(from)}|${escapeSedLiteral(to)}|g`
  )
...
`
  ...
  const applySrc = `${hdr}echo "Previewing src normalization moves from var/plan/src-move-map.tsv"
while IFS=$'\\t' read -r FROM TO; do
  [ -z "$FROM" ] && continue
  echo git mv "$FROM" "$TO"
  git add -N "$FROM" "$TO" >/dev/null 2>&1 || true
...
`
  ...
  const applyBuild = `${hdr}echo "Suggested Eleventy passthroughs (verify in your project config):"
${passthroughs.map(p => `echo "  - ${p}"`).join('\n')}
`

  await fsp.writeFile(path.join(planDir, 'apply-moves.sh'), applyMoves, {
    mode: 0o755,
  })
  await fsp.writeFile(path.join(planDir, 'apply-src.sh'), applySrc, {
    mode: 0o755,
  })
  await fsp.writeFile(path.join(planDir, 'apply-rewrites.sh'), applyRewrites, {
    mode: 0o755,
  })
  await fsp.writeFile(path.join(planDir, 'apply-build.sh'), applyBuild, {
    mode: 0o755,
  })
}
...
async function writeReports(
  ...
  const md = [
    ...
    movesDocs.length
      ? movesDocs.map(m => `- ${m.replace('\t', ' -> ')}`).join('\n')
      : '- None',
    '## Src Normalization Moves (CSS)',
    srcPlan.moves.length
      ? srcPlan.moves.map(m => `- ${m.replace('\t', ' -> ')}`).join('\n')
      : '- None',
    '## Suggested Rewrites',
    rewrites.length
      ? rewrites.map(([f, t]) => `- \`${f}\` → \`${t}\``).join('\n')
      : '- None',
    ...
    srcPlan.conflicts.redundantDuplicates.length
      ? [
          ...
          ...srcPlan.conflicts.redundantDuplicates.map(
            d => `- keep: \`${d.keep}\` — redundant: \`${d.redundant}\``
          ),
        ].join('\n')
      : '### Redundant duplicates (identical content)\n- None',
    srcPlan.conflicts.contentConflicts.length
      ? [
          '### Content conflicts (different content parked under _conflicts)',
          ...srcPlan.conflicts.contentConflicts.map(
            c =>
              `- name: \`${c.name}\`\n  - existing: \`${c.existing}\`\n  - parked: \`${c.parked}\`\n  - from: \`${c.from}\``
          ),
        ].join('\n')
    ...
    srcPlan.conflicts.skipped.length
      ? [
          '### Skipped (not moved)',
          ...srcPlan.conflicts.skipped.map(s => `- ${s}`),
        ].join('\n')
      : '### Skipped\n- None',
    '## Edge Cases',
    '- None observed.',
