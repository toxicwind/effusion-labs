...
test('collage style includes base, ephemera, and lab modules', async () => {
  ...
  let rafCalls = 0;
  dom.window.requestAnimationFrame = dom.window.requestAnimationFrame || ((cb) => { if (rafCalls++) return 0; setTimeout(() => cb(Date.now()), 0); return rafCalls; });
  ...
  assert.ok(doc.querySelector('.mschf-tape, .mschf-stamp, .mschf-quotes, .mschf-plate, .mschf-specimen'));
  // Lab elements: when GPU not available, DOM variants are used
  assert.ok(doc.querySelector('.mschf-callout, .mschf-graph, .mschf-rings, .mschf-topo, .mschf-halftone, .mschf-perf, .mschf-stars'));
});
...
test('auto style mounts overlay in auto mode', async () => {
  ...
  assert.ok(doc.querySelector('.mschf-grid'));
  assert.ok(
    doc.querySelector('.mschf-tape, .mschf-stamp, .mschf-quotes, .mschf-plate, .mschf-specimen, .mschf-callout, .mschf-graph, .mschf-rings, .mschf-topo, .mschf-halftone, .mschf-perf, .mschf-stars')
  );
});
