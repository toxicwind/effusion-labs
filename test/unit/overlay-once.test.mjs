...
function runOverlayInJsdom(html = '') {
  ...
  class IO {
    ...
    unobserve(t) {
      ...
      this.unobs.set(t, (this.unobs.get(t) || 0) + 1)
    }
    disconnect() {
      this.targets.clear()
    }
    trigger(target, isIntersecting = true) {
      this.cb([{ target, isIntersecting }], this)
    }
  }
  ...
  const code = fs.readFileSync('js/mschf-overlay.js', 'utf8')
...
test('overlay section mounts fire only once per target', () => {
  ...
  assert.equal(
    afterSecond,
    afterFirst,
    'second pass should not add more elements'
  )
})
