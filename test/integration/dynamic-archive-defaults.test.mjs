...
test('A1–A3: wikilinks resolve to canonical dynamic archive routes', async () => {
  ...
  assert.match(
    html,
    /href="\/archives\/series\/lets-checkmate\//,
    'series link canonical href present'
  )

  // Character → /archives/character/momo-fox/
  assert.match(
    html,
    /href="\/archives\/character\/momo-fox\//,
    'character link canonical href present'
  )

  // Product → /archives/product/tempura-shrimp/
  assert.match(
    html,
    /href="\/archives\/product\/tempura-shrimp\//,
    'product link canonical href present'
  )
})
