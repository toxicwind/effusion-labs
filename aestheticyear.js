...
(function ($) {
  ...
  `).appendTo(document.body);
    ...
    function gqlShortcodeHash(shortcode) {
        console.log(`[IGM][DEBUG] Starting gqlShortcodeHash for shortcode: ${shortcode}`);
        ...
        return new Promise((resolve, reject) => {
            GM_xmlhttpRequest({
                ...
                url,
                onload: (r) => {
                    console.log(`[IGM][DEBUG] Hash response status: ${r.status}, statusText: ${r.statusText}`);
                    console.log(`[IGM][DEBUG] Hash response text: ${r.responseText.substring(0, 500)}...`);
                    ...
                    try {
                        ...
                        if (node) {
                            const tsRaw = (typeof node.taken_at_timestamp === 'number') ? node.taken_at_timestamp : null;
                            console.log(`[IGM][DEBUG] Hash taken_at_timestamp: ${node.taken_at_timestamp}, tsRaw: ${tsRaw}`);

                            const result = {
                                ts: Number.isFinite(tsRaw) ? tsRaw * 1000 : Number.NaN,
                                ts_source: tsRaw != null ? 'hash.taken_at_timestamp' : 'hash.none',
                                ts_raw: tsRaw,
                                img: bestFromResources(node.display_resources) || node.display_url || null,
                                img_source: node.display_resources ? 'hash.display_resources' : (node.display_url ? 'hash.display_url' : 'hash.none')
                            };
                            console.log(`[IGM][DEBUG] Hash result:`, result);
                            resolve(result);
                        } else if (obj?.status === 'fail') {
                            console.log(`[IGM][DEBUG] Hash API failure: ${obj.message || 'fail'}`);
    ...
    function gqlShortcodeQueryId(shortcode) {
        console.log(`[IGM][DEBUG] Starting gqlShortcodeQueryId for shortcode: ${shortcode}`);
        ...
        return new Promise((resolve, reject) => {
            GM_xmlhttpRequest({
                ...
                headers,
                onload: (r) => {
                    console.log(`[IGM][DEBUG] QueryId response status: ${r.status}, statusText: ${r.statusText}`);
                    console.log(`[IGM][DEBUG] QueryId response text: ${r.responseText.substring(0, 500)}...`);

                    try {
                        const obj = JSON.parse(r.responseText);
                        console.log(`[IGM][DEBUG] QueryId parsed object:`, obj);

                        const node = obj?.data?.xdt_api__v1__media__shortcode__web_info?.items?.[0];
                        console.log(`[IGM][DEBUG] QueryId node:`, node);

                        if (node) {
                            const tsPicked = (typeof node.taken_at === 'number') ? node.taken_at : (typeof node.taken_at_timestamp === 'number') ? node.taken_at_timestamp : null;
                            const tsSource = (typeof node.taken_at === 'number') ? 'id.taken_at' : (typeof node.taken_at_timestamp === 'number') ? 'id.taken_at_timestamp' : 'id.none';
                            console.log(`[IGM][DEBUG] QueryId taken_at: ${node.taken_at}, taken_at_timestamp: ${node.taken_at_timestamp}`);
                            console.log(`[IGM][DEBUG] QueryId tsPicked: ${tsPicked}, tsSource: ${tsSource}`);

                            let imgUrl = null, imgSrc = 'id.none';
    ...
    function pump() {
        ...
        while (inflight < CONCURRENCY && queue.length) {
            ...
            console.log(`[IGM][DEBUG] Starting hydration for shortcode: ${sc}, inflight: ${inflight}`);
            inflight++;
            hydrate(sc).finally(() => {
                inflight--;
                console.log(`[IGM][DEBUG] Completed hydration for shortcode: ${sc}, inflight now: ${inflight}`);
                render();
                if (DEBUG_OPEN) renderDebug();
                pump();
            });
    ...
    async function hydrate(shortcode) {
        ...
        if (Number.isFinite(rec.ts) && rec.img) {
            console.log(`[IGM][DEBUG] Shortcode ${shortcode} already has ts: ${rec.ts} and img: ${rec.img ? 'yes' : 'no'}`);
            return;
        }

        console.log(`[IGM][DEBUG] Shortcode ${shortcode} needs hydration - ts: ${rec.ts}, img: ${rec.img ? 'yes' : 'no'}`);
        ...
        try {
            ...
            if (!items.get(shortcode).img) {
                console.log(`[IGM][DEBUG] Still no img for ${shortcode}, trying Hash fallback`);
                try {
            ...
            console.log(`[IGM][DEBUG] QueryId failed for ${shortcode}, trying Hash:`, e);
            try {
                merge(shortcode, await gqlShortcodeHash(shortcode));
            } catch (e2) {
                console.log(`[IGM][DEBUG] Both APIs failed for ${shortcode}. QueryId:`, e, 'Hash:', e2);
            }
        }
    }
    ...
    function merge(shortcode, data) {
        ...
        if (Number.isFinite(data.ts) && !Number.isFinite(rec.ts)) {
            console.log(`[IGM][DEBUG] Setting ts for ${shortcode}: ${data.ts} (source: ${data.ts_source})`);
            rec.ts = data.ts;
            rec.ts_source = data.ts_source || 'unknown';
            rec.ts_raw = (typeof data.ts_raw !== 'undefined') ? data.ts_raw : (Number.isFinite(data.ts) ? Math.round(data.ts / 1000) : null);
        }
    ...
    function findDomTimeForShortcode(sc) {
        ...
        try {
            ...
            console.log(`[IGM][DEBUG] Closest article: ${$article.length}, closest div: ${$div.length}`);

            const $time = $a.closest('article,div').find('time[datetime]').first();
            console.log(`[IGM][DEBUG] Found ${$time.length} time element(s) with datetime`);
            ...
            if ($time.length) {
                ...
                console.log(`[IGM][DEBUG] Parsed date: ${ms}, isFinite: ${Number.isFinite(ms)}`);
                ...
                console.log(`[IGM][DEBUG] Found ${$timeAll.length} time element(s) total`);
                $timeAll.each((i, el) => {
                ...
                const $dateElements = $a.closest('article,div').find('[data-timestamp], [data-time], [title*="202"], [title*="201"]');
                console.log(`[IGM][DEBUG] Found ${$dateElements.length} potential date elements`);
    ...
    function scanOnce() {
        ...
        console.log(`[IGM][DEBUG] Found ${anchors.length} Instagram post/reel anchors`);
        ...
        for (const href of dedupe(anchors)) {
            ...
            if (!sc) {
                console.log(`[IGM][DEBUG] Could not extract shortcode from href: ${href}`);
                continue;
            }
            ...
            if (!rec) {
                ...
                console.log(`[IGM][DEBUG] Found ${$img.length} img element(s) for ${sc}`);

                const srcset = $img.attr('srcset');
                const src = $img.attr('src');
                console.log(`[IGM][DEBUG] Img srcset: ${srcset ? 'present' : 'none'}, src: ${src ? 'present' : 'none'}`);
                ...
                if (dt) {
                    ...
                    console.log(`[IGM][DEBUG] Set DOM time for ${sc}: ${dt.ms} (${dt.iso})`);
                } else {
    ...
    function render() {
        ...
        for (const it of arr) {
            ...
            const $year = $('<div class="igmjq-year">').addClass(bucket).attr('data-year', yr || '');
            $year.append('<span class="dot"></span>').append(document.createTextNode(yr ? String(yr) : '—'));
            $a.append($year);
