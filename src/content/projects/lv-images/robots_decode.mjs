...
function guessExampleFromPattern(val, origin) {
  ...
  if (/\/exclusive-access\/?$/.test(v)) {
    const u = new URL(base)
    u.pathname = '/exclusive-access/'
    return u.toString()
  }
  if (/\/eng-us\/mylv\/?$/.test(v)) {
    const u = new URL(base)
    u.pathname = '/eng-us/mylv/'
    return u.toString()
  }
  if (/\/mylv\/newsletter/i.test(v)) {
    const u = new URL(base)
    u.pathname = '/mylv/newsletter'
    return u.toString()
  }
  if (/page-\*/i.test(v)) {
    const u = new URL(base)
    u.pathname = '/collection/page-2'
    return u.toString()
  }
  if (/lvxmurakami/i.test(v)) {
    const u = new URL(base)
    u.pathname = '/collections/lvxmurakami'
    return u.toString()
  }
...
export function decodeRobots(robotsText, origin) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (directive === 'user-agent') {
      ...
      directive === 'allow' || directive === 'disallow' || directive === 'noindex'
