...
export default async function (data = {}) {
  ...
  for (const [absJsonl, pdata] of provToProduct.entries()) {
    ...
    for (const e of entries) {
      ...
      if (h) {
        const hs = push(hosts, h, () => ({ host: h, entries: 0, products: new Set(), first: null, last: null }));
        hs.entries += 1;
      ...
      if (ccy) {
        const cs = push(currencies, ccy, () => ({ code: ccy, entries: 0, min: null, max: null, products: new Set() }));
        cs.entries += 1;
      ...
      if (mkt) {
        const ms = push(markets, mkt, () => ({ code: mkt, entries: 0, products: new Set() }));
        ms.entries += 1;
