...
; (() => {
  ...
  function mountRoot() {
    ...
    css(root, {
      pointerEvents: 'none', userSelect: 'none', position: 'fixed',
      inset: '0', zIndex: '44', contain: 'layout style paint', opacity: '0.9',
    });
    ...
    css(State.domLayer, { position: 'absolute', inset: 0, pointerEvents: 'none', userSelect: 'none' });
  ...
  function analyzePageLayout() {
    ...
    document.querySelectorAll(selector).forEach(node => {
      ...
      if (r && r.width > 0 && r.height > 0) {
        rects.push({
          x: clamp(r.left - pad, 0, innerWidth), y: clamp(r.top - pad, 0, innerHeight),
          w: clamp(r.width + pad * 2, 0, innerWidth), h: clamp(r.height + pad * 2, 0, innerHeight),
        });
      }
    });
  ...
  const rectOverlap = (a, b) => Math.max(0, Math.min(a.x + a.w, b.x + b.w) - Math.max(a.x, b.x)) * Math.max(0, Math.min(a.y + a.h, b.y + b.h) - Math.max(a.y, b.y));
  ...
  A.grid = () => ({
    kind: 'grid', cost: 1,
    mount(p) { this.node = el('div', 'mschf-grid', p); this.node.dataset.variant = pick(['dots', 'lines']); }
  });
  A.rulers = () => ({
    kind: 'rulers', cost: 1,
    mount(p) {
      ...
      this.node = { remove: () => { top.remove(); left.remove(); } };
    }
  });
  A.corners = () => ({
    kind: 'corners', cost: 1,
    mount(p) {
      const nodes = ['tl', 'tr', 'bl', 'br'].map(pos => el('div', `mschf-corner mschf-corner-${pos}`, p));
  ...
  A.specimen = () => ({
    ...
    mount(p) {
      ...
      this.node.innerHTML = `<strong>SPECIMEN</strong><span>ID ${id}</span><span>${new Date().toISOString().slice(0, 10)}</span>`;
  ...
  A.edgeGlow = () => ({
    kind: 'edgeGlow', cost: 1,
    mount(p) {
      this.nodes = ['top', 'right', 'bottom', 'left'].map(pos => el('div', `mschf-edge mschf-edge-${pos}`, p));
  ...
  window.__mschfOff = () => { localStorage.setItem('mschf:off', '1'); State.root?.remove(); };
