...
;(() => {
  ...
  const GPU = {
    ...
    updateMask() {
      ...
      const PIXI =
        this.app.renderer.plugins?.graphics?.graphicsFactory?.renderer?.PIXI ||
        globalThis.PIXI
      if (!PIXI) return
      ...
      update() {
        if (Math.random() < 0.0015 * State.tempo)
          node.dataset.variant =
            node.dataset.variant === 'dots' ? 'lines' : 'dots'
      },
      node,
    }
  }
  ...
  A.tape = () => {
    ...
    return {
      ...
      mount(p) {
        ...
        node.textContent = `${pick(TAPE_LEX)} • RIG-${Math.floor(
          Math.random() * 0xffff
        )
          .toString(16)
          .padStart(4, '0')
        ...
        const rot =
          (Math.random() - 0.5) * (/(loud|storm)/.test(State.mood) ? 12 : 5)
        css(node, {
          top: rect.y + 'px',
          left: rect.x + 'px',
          width: wvw + 'vw',
  ...
  A.callout = () => {
    ...
    return {
      ...
      update(t) {
        ...
        if (Math.random() < 0.02)
          node.style.filter = `drop-shadow(0 0 ${Math.random() < 0.5 ? 4 : 6}px color-mix(in oklab, currentColor 40%, transparent))`
      },
      node,
    }
  }
    ...
    return {
      ...
      update() {
        ...
        for (const s of nodes) {
          const x =
            parseFloat(s.style.getPropertyValue('--x') || '0vw') ||
            Math.random() * 100
          const y =
            parseFloat(s.style.getPropertyValue('--y') || '0vh') ||
            Math.random() * 100
          const nx = clamp(
  ...
  A.brackets = () => {
    ...
    return {
      ...
      mount(p) {
        ...
        node.dataset.variant =
          State.readingPressure > 0.18 || isArticle ? 'lite' : 'bold'
  ...
  A.watermark = () => {
    ...
    return {
      ...
      mount(p) {
        ...
        const isArticle = !!document.querySelector('.prose,article,main .prose')
        const variant =
          State.readingPressure > 0.25 || isArticle
            ? 'stripe'
            : pick(['stripe', 'full'])
  ...
  A.reg = () => {
    ...
    return {
      ...
      mount(p) {
        ;['tl', 'tr', 'bl', 'br'].forEach(pos =>
          nodes.push(el('div', 'mschf-reg ' + pos, p))
        )
      },
      node: {
        remove() {
