...
export default function register(eleventyConfig) {
  ...
  // Expose raw archives (JSON + JSONL) under /content/archives/** for direct access
  // Copy from the repo path (src/content/archives) to site path (content/archives)
  eleventyConfig.addPassthroughCopy({ "src/content/archives": "content/archives" });
  ...
  if (eleventyConfig.setServerOptions) {
    const serveJsonl = process.env.ELEVENTY_SERVE_JSONL !== "0";
    const onRequest = serveJsonl
      ? {
          "/content/:rest*": ({ url }) => {
            if (url.pathname.endsWith(".jsonl")) {
              return { headers: { "Content-Type": "application/x-ndjson; charset=utf-8" } };
            }
          },
        }
      : {};
