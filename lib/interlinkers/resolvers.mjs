...
function guessHref(kind, nameSlug) {
  const def = routeRegistry.kinds[kind];
  const locale = routeRegistry.defaultLocale;
  const maybePrefix = routeRegistry.localePrefixEnabled ? `/${locale}` : '';
  return `${maybePrefix}${def.basePath}/${nameSlug}/`;
}
...
function resolverFor(kind) {
  ...
  return (link, currentPage /*, interlinker */) => {
    try {
      ...
      const href = entry?.href || guessHref(kind, wantSlug);
      ...
      const wantSlug = slugify(link?.name);
      const href = guessHref(kind, wantSlug);
...
function dispatcherForOmittedKind() {
  ...
  return (link, currentPage /*, interlinker */) => {
    ...
    const href = guessHref(order[0], nameSlug);
    const label = escapeHtml(link?.title || link?.name);
