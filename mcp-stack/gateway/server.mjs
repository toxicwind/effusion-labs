    ...
    if (m) {
          ...
          req.on('end', async () => {
            ...
            const run = async () => {
              try {
                ...
                return {
                  status: 400,
                  body: { error: 'info_unsupported_for_server' },
                }
              } catch (e) {
                return {
                  status: 200,
      ...
      if (action === 'sse') {
        ...
        try {
          ...
          log('error', 'spawn', 'failed', { server: name, err: String(e) })
        }
      ...
      if (action === 'send') {
        ...
        req.on('end', async () => {
          try {
            ...
            return json(res, 400, {
              error: 'invalid_json_or_send_failed',
              detail: String(e?.message || e),
            })
          }
        })
...
async function sidecarStatus(nowIso) {
  ...
  items.push({
    name: 'screenshot',
    url: 'http://screenshot:9340',
    version: '2.0.1',
    health: 'ready',
    capabilities: {
      deterministicRaster: true,
      formats: ['png', 'jpeg', 'webp'],
      viewportConfigurable: true,
    },
    lastChecked: nowIso,
  })
  items.push({
...
async function probeFlare(nowIso) {
  ...
  try {
    ...
    return {
      ...
      version: 'unknown',
      health: 'degraded',
      capabilities: { cloudflareBypass: false },
      lastChecked: nowIso,
  ...
  return {
    ...
    admin: {
      queue: {
        schema: {
          type: 'object',
          required: ['currentLength', 'avgWaitMs', 'maxConcurrency'],
          properties: {
            currentLength: { type: 'integer', minimum: 0 },
            avgWaitMs: { type: 'integer', minimum: 0 },
      ...
      rate: {
        schema: {
          type: 'object',
          required: ['limitPerSec', 'burst'],
          properties: {
            limitPerSec: { type: 'integer', minimum: 0 },
      ...
      retry: {
        schema: {
          type: 'object',
          required: ['policy', 'baseMs', 'maxMs'],
          properties: {
            policy: { type: 'string', enum: ['exponential', 'linear', 'none'] },
