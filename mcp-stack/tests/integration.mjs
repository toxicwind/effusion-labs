...
async function main() {
  ...
  try {
    ...
    const sseRes = await fetch(`${base}/servers/filesystem/sse`, {
    ...
    if (process.env.CI === '1') {
      ...
      const r = await fetch(`${base}/servers/readweb/info`, {
        method: 'POST',
    ...
    if (process.env.INTERNET_TESTS === '1') {
      ...
      const readWebRes = await fetch(`${base}/servers/readweb/info`, {
        method: 'POST',
