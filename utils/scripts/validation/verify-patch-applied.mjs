...
async function verifyPatch(patchFile) {
    const patchContent = await fse.readFile(path.join(PATCHES_DIR, patchFile), 'utf-8');
    const lines = patchContent.split('\n');

    // Heuristic: Find the first file path and the first added line of code to use as a check.
    const targetFileRelativePathLine = lines.find(l => l.startsWith('--- a/node_modules/'));
    const sentinelLineAdded = lines.find(l => l.startsWith('+') && !l.startsWith('+++'));

    if (!targetFileRelativePathLine) {
        return { success: false, patchFile, reason: 'Could not determine target file from patch.' };
    }
    if (!sentinelLineAdded) {
        return { success: false, patchFile, reason: 'Could not find a sample added line in patch to verify.' };
    }
    ...
    if (!await fse.exists(fullTargetPath)) {
        return { success: false, patchFile, targetFile, sentinelLine, reason: 'Target file does not exist in node_modules.' };
    }

    const targetContent = await fse.readFile(fullTargetPath, 'utf-8');
    if (!targetContent.includes(sentinelLine)) {
        return { success: false, patchFile, targetFile, sentinelLine, reason: 'Patched content not found in target file.' };
    }
