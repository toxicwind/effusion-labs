...
function codeframe(srcText, line = 1, col = 1, ctx = 3) {
    ...
    for (let n = start; n <= end; n++) {
        const pref = (n === line ? ">" : " ") + " " + String(n).padStart(width) + " | ";
        out.push(pref + lines[n - 1]);
        if (n === line && col && col > 0) out.push(" ".repeat(pref.length + col - 1) + "^");
    }
    return out.join("\n");
}

function rel(p) {
    try { return path.relative(ROOT, path.resolve(p)); } catch { return p; }
}
...
function extractNunjucksLocation(stderrText) {
    ...
    const re1 = /\((?<file>[^)]+)\)\s*\[Line\s*(?<line>\d+),\s*Column\s*(?<col>\d+)\]/;
...
function extractPermalinkConflict(stderrText) {
    const head = /Output conflict: multiple input files are writing to\s+`(?<target>[^`]+)`/;
...
function findPermalinkLine(filePath) {
    try {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (inFm && L.trim() === "---") { inFm = false; break; }
...
async function runTernaryFixer() {
    ...
    try {
        const { fixRepoTernaries } = await import(path.join(ROOT, "config", "fix-njk-ternaries.mjs"));
        ...
        if (!report.quiet && report.modified > 0) {
            banner(`ternary patched ${report.modified} file(s); log → ${report.logFile}`);
        }
    } catch (e) {
    ...
    proc.on("close", async (code) => {
        ...
        if (dup?.files?.length >= 2) {
            ...
            for (const f of dup.files) {
                ...
                console.error(` - ${rel(abs)}${hint ? `  (permalink @ line ${hint.line}: ${hint.preview})` : ""}`);
            }
